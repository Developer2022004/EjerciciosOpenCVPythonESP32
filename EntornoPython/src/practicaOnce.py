...
while(True):
    try:
        ...
        figura, grafico_grid = plt.subplots(2,2,figsize=(10,4))
        ax = grafico_grid[0,0]
        bx = grafico_grid[0,1]
        cx = grafico_grid[1,0]
        dx = grafico_grid[1,1]
        ...
        cx.plot(range(len(lista)),lista,marker="o",linestyle='--')
        cx.set_ylim(0,200)
        cx.set_title('Grafico de puntos y lineas en OpenCV')
        cx.set_xlabel('Sensor')
        cx.set_ylabel('Valores')
        
        #Grafico de lineas
        dx.plot(range(len(lista)),lista)
        dx.set_xlabel('Sensor')
        dx.set_ylabel('Valores')
        dx.set_title('Grafico de lineas')
        dx.set_ylim(0,200)
